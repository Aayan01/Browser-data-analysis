...
from deep_translator import GoogleTranslator #convert into prefered language 
...
# Input Video Link
url = st.sidebar.text_input('Video URL', 'https://www.youtube.com/watch?v=9z8ujpPgUjI')
...
> Success!
    ...
    # Translate and Print 
    translated = GoogleTranslator(source='auto', target= get_key_from_dict(add_selectbox,languages_dict)).translate(transcript)
    html_str3 = f"""
    <style>
    p.a {{
    text-align: justify;
    ...
    if add_selectbox in no_support:
        ...
        audio_file = open('user_trans.mp3', 'rb')   #rb : Opens the file as read-only in binary format and starts reading from the beginning of the file.    
